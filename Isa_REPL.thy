...
ML_file ‹contrib/mlmsgpack/mlmsgpack-aux.sml›
ML_file ‹contrib/mlmsgpack/realprinter-packreal.sml›
ML_file ‹contrib/mlmsgpack/mlmsgpack.sml›

ML_file ‹library/premise_selection.ML›
ML_file ‹library/REPL.ML›
ML_file ‹library/REPL_serializer.ML›
ML_file ‹library/REPL_aux.ML›
ML_file ‹library/Server.ML›

(*
ML ‹Config.declare_option_bool›
...
let fun mk_idt s = Token.make ((2,0),s) Position.none |> fst
       ...
       |> snd
       |> (fn thy => Syntax.string_of_term_global thy @{prop ‹(1::nat) + 1 = 2›})
       |> tracing
      end
›
...
*)
(*
ML ‹Symtab.lookup (Attrib.Configs.get @{theory}) name›
...
(*
ML ‹Symbol_Pos.explode ("asdasd", Position.none)
  |> Vector.fromList
             |> Vector.map fst
...
ML ‹(REPL_Aux.column_of_pos "ML ‹\nx›" |> snd)
        (Position.make0 2 6 1 "" "" "")›
*)

(*
ML ‹REPL_Serialize.string_of_term "T4S4" (Context.Proof \<^context>) (Thm.prop_of @{thm allI})›



ML ‹REPL_Serialize.print_term {all_typ=true, bv_typ=true, constant_typ=true,
        free_typ=true, num_typ=true, sorting=true, show_type_P=SOME 0.4, show_sort_P=SOME 0.5}
    \<^context> (Thm.prop_of @{thm allI}) ›

ML ‹Random.random ()›
ML ‹Time.now ()›
ML ‹fun rmod x y = x - y * Real.realFloor (x / y);› 
